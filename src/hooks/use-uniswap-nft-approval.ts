...
const UNISWAP_V3_NFT_ADDRESS = "0x3dCc735C74F10FE2B9db2BB55C40fbBbf24490f7" as Address;
...
export function useUniswapNFTApproval(tokenId?: string) {
  ...
  const { data: isApprovedForAll, isLoading: checkingApproval } = useReadContract({
  ...
  const approveAll = async () => {
    ...
    try {
      ...
      setState(prev => ({
        ...prev,
        error: error instanceof Error ? error.message : "Failed to approve NFTs",
        isApproving: false,
      }));
    }
  };
